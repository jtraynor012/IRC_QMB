...
class Server():
	...
	def mainLoop(self):
		...
		while True:
			...
			client_thread = threading.Thread(target=self.init_client, args=[client, address])
			client_thread.start()
	...
	def init_client(self, client, address):
		self.client_socket_list.append(client)
		self.client_list.append(Client(client, address))
	...
	def commands(self, client_socket):
		...
		while True:
			...
			print(f'{client.getAddress()} --> {data.encode()}')
	...
	#do nothing function to handle CAP on welcome
	def cap(self):
		return
	...
	def nick(self, cmd, client):
		...
		if len(cmd) < 2:
			...
			x = re.search("^[a-zA-Z0-9\-\_\\[\]\{\}\|]{1,10}$", cmd[1])
	...
	def privmsg(self, cmd, client):
		...
		msg = f':{client.getNick()}!{client.getUser()}@{self.hostname} PRIVMSG {target} '
		for m in cmd[2:]:
			msg += ' ' + m
		...
		if target[0] == '#':
			...
			for i in self.channels:
				if i.getName() == target:
					for cli in i.getClients():
							#makes sure doesnt send message to self
							if cli.getNick() != client.getNick():
								self.socketMessage(cli, msg.encode()) 

			##ERR_NOSUCHCHANNEL
			self.socketMessage(client, f'{self.hostname} 403 <{target}> :No such channel\r\n'.encode())
			return
		##single user message mode
			...
			for i in self.client_list:
				if i.getNick() == target:
					self.socketMessage(i, msg.encode())
					return

			##ERR_NOSUCHNICK
			self.socketMessage(client, f'{self.hostname} 401 <{target}> :No such nick/channel')
			return
	...
	def who(self, cmd, client):
		...
		if target[0] == '#':
			...
			for i in self.channels:
				if i.getName() == target:
					for cli in i.getClients():
						msg = f':{self.hostname} 352 {client.getNick()} {target} {cli.getUser()} {cli.getAddress()} {self.hostname} {cli.getNick()} H :0 {cli.getRealname()}\r\n'.encode()
						self.socketMessage(client, msg)
					self.socketMessage(client, f'{self.hostname} 315 {client.getNick()} :<{target}> :End of WHO list\r\n'.encode())	
